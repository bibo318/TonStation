...
class TonStation:
    def __init__(self, thread: int, session_name: str, phone_number: str, proxy: [str, None]):
        ...
        self.sp = 'ref_tufd9mvw5gdrmzbabwm6mg'
    ...
    async def login(self):
        ...
        while attempts:
            try:
                ...
                logger.success(f"Chủ đề {self.thread} | {self.account} | Đăng nhập")
                break
            except Exception as e:
                logger.error(f"Chủ đề {self.thread} | {self.account} | Số lần đăng nhập còn lại: {attempts}, lỗi: {e}")
                await asyncio.sleep(random.uniform(*config.DELAYS['RELOGIN']))
                attempts -= 1
        else:
            logger.error(f"Chủ đề {self.thread} | {self.account} | Không thể đăng nhập")
            await self.logout()
            return
